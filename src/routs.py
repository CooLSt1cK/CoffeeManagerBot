...
async def shift_started(update: Update, context: CallbackContext):
    if update.message.text == 'Розпочати робочій день':
        ...
        total_caps_xs = list(filter(lambda cap: cap['name'] == 'XS', context.user_data['shift']['caps']))[0] - sum(
            map(lambda order: 1,
                filter(lambda order: 'size' in order and order['size']['name'] == 'XS', order_list_total)))
        total_caps_s = list(filter(lambda cap: cap['name'] == 'S', context.user_data['shift']['caps']))[0] - sum(
            map(lambda order: 1,
                filter(lambda order: 'size' in order and order['size']['name'] == 'S', order_list_total)))
        total_caps_m = list(filter(lambda cap: cap['name'] == 'M', context.user_data['shift']['caps']))[0] - sum(
            map(lambda order: 1,
                filter(lambda order: 'size' in order and order['size']['name'] == 'M', order_list_total)))
        total_caps_l = list(filter(lambda cap: cap['name'] == 'L', context.user_data['shift']['caps']))[0] - sum(
            map(lambda order: 1,
                filter(lambda order: 'size' in order and order['size']['name'] == 'l', order_list_total)))
        total_caps_xl = list(filter(lambda cap: cap['name'] == 'XL', context.user_data['shift']['caps']))[0] - sum(
            map(lambda order: 1,
