...
async def shift_started(update: Update, context: CallbackContext):
    if update.message.text == 'Розпочати робочій день':
        ...
        if str(update.effective_user.id) in current_state['users']:
            context.user_data.update(current_state['users'][str(update.effective_user.id)])
            await update.message.reply_text('Створіть замовлення)', reply_markup=ReplyKeyboardMarkup(
        ...
        await update.message.reply_text(
            'Вкажіть кількість стаканчиків через кому в наступному порядку:\nВеликий лате,s-лате,капучино,американо,еспресо')
        ...
        xl, l, m, s, xs = list(map(int, update.message.text.split(',', 5)))
